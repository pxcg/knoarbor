...
def _require_vault(runtime: Runtime) -> str:
    if not runtime.vault_path:
        lines = ["KnoArbor vault path is required. Run `check`, pass --vault/--vault-id, or use a service that exposes /runtime."]
        if runtime.vault_id:
            lines.append(f"Requested vault ID: {runtime.vault_id}")
        if runtime.vaults:
            available = ", ".join(
                f"{item.get('id')} ({item.get('name') or item.get('path')})"
                for item in runtime.vaults
                if isinstance(item, dict) and item.get("id")
            )
            if available:
                lines.append(f"Available vault IDs: {available}")
