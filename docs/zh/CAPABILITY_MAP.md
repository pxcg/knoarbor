...
| 知识编译 | 已实现 | Ingest 冻结标准化输入，建立 source units 和不可变事实 revision，原子发布 active heads，再物化可重建的 Wiki/索引投影与报告。 | [架构设计](ARCHITECTURE.md), [核心概念](CONCEPTS.md) |
...
| Query 上下文检索 | 已实现 | Query 排序 active knowledge atoms，选择可回答 claim，沿显式 evidence edge 解析完整 active raw source units，并返回稳定证据身份、缺口、trace 和可选投影定位；它不对 raw 正文评分，也不把页面当成事实权威。 | [1.38 Semantic Indexed Raw Query](../../specs/1.38-semantic-indexed-raw-query/requirements.md), [接口说明](API.md) |
| 运行执行与监控 | 已实现 | Runtime 负责持久化事务任务状态、进程内调度、心跳、取消、恢复元数据和 active/recent run 视图；当前不依赖独立的持久 worker queue。 | [架构设计](ARCHITECTURE.md), [接口说明](API.md) |
| 报告与审计层 | 已实现 | Audit 负责 ingest、lint、query、token 和失败报告，以及机器可读 ledger。 | [架构设计](ARCHITECTURE.md), [测试与质量门禁](TESTING.md) |
| 模型网关 | 已实现 | 模型调用经过 provider adapter、OpenAI 兼容或 Ollama 原生 transport、结构化输出处理、usage metrics 和 endpoint 检查。 | [架构设计](ARCHITECTURE.md), [配置说明](CONFIGURATION.md) |
...
| 前端 i18n 约束 | 已实现 | UI 文案集中在 `renderer/src/i18n/` 下；前端构建前检查中英文 key 一致性。 | [测试与质量门禁](TESTING.md) |
...
| 公开 API | 已实现 | 公开 endpoint family 通过稳定领域契约覆盖 health/doctor、vaults/config/models、sources、ingest/lint/query、chat、runs/reports、wiki pages、tokens 和 runtime metadata。 | [接口说明](API.md), [API 兼容性](API_COMPATIBILITY.md) |
...
| 机器索引层 | 部分实现 | 由 `.knoarbor/index/CURRENT` 选择的不可变索引 generation 服务页面与 graph 导航。默认 query 从事实存储读取 active semantic atoms 和显式 claim-evidence edges；更多 provider 选择与 freshness 诊断仍未完成。 | [1.4 Machine Index Layer](../../specs/1.4-machine-index-layer/requirements.md), [1.38 Semantic Indexed Raw Query](../../specs/1.38-semantic-indexed-raw-query/requirements.md) |
