...
| `KA-VAULT-001` | `user_input_error` | 400 | 否 | vault、wiki 页面或 ledger 路径无效，或逃逸出其配置根目录。 | 使用配置 vault 内部路径，避免绝对路径或上级目录穿越写入 wiki。 |
...
| `KA-POLICY-001` | `policy_rejection` | 422 | 否 | 请求的编辑或生成结果违反了 KnoArbor 的策略不变量。 | 查看错误详情，并按需调整来源内容、请求、提示词契约或策略。 |
