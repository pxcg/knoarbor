...
- `vaults/all/raw/chats/`: AI tool sessions such as Hermes, Codex, OpenClaw, or Claude Code.
- `vaults/all/raw/notes/`: imported Markdown notes.
- `vaults/all/raw/articles/`: web or article exports.
- `vaults/all/raw/documents/originals/`: original rich documents such as PDF, DOCX, PPTX, XLSX, manuals, and course material.
- `vaults/all/raw/documents/markdown/`: Markdown generated by deterministic preprocessors such as a user-managed MinerU-compatible service.
- `vaults/all/raw/transcripts/`: meeting, audio, or video transcripts.
- `vaults/all/raw/datasets/`: structured datasets.
- `vaults/all/raw/media/`: original images and media files.
...
The knowledge layer stores maintained wiki pages under `vaults/all/pages/`.
Open `vaults/all/pages` in Obsidian when you want a clean vault without raw sources,
...
Human-readable reports stay in `vaults/all/maintenance/`. Machine state, runs,
ledgers, checkpoints, locks, and indexes stay in `vaults/all/.knoarbor/`.
...
- generated `vaults/all/pages/index.md` for human-readable routing and debugging;
...
- lexical matches across title, path, tags, summary, key points, headings, and body;
