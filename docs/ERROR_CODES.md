...
| `KA-VAULT-001` | `user_input_error` | 400 | No | A vault, wiki page, or ledger path is invalid or escapes its configured root. | Use a path under the configured vault and avoid absolute/parent traversal paths for wiki writes. |
...
| `KA-POLICY-001` | `policy_rejection` | 422 | No | A requested edit or generated result violated a KnoArbor policy invariant. | Inspect the error details and adjust the source, request, prompt contract, or policy if the rejection is expected. |
