...
| Desktop renderer | Partial | The React renderer is the primary desktop UI for sources, ingest, lint, query, wiki browsing, graph, reports, runs, settings, and token analysis. The Python-hosted browser console is a development fallback during the desktop-first transition. | [1.15 Desktop App](../specs/1.15-desktop-app/requirements.md), [1.20 Desktop-First Transition](../specs/1.20-desktop-first-transition/requirements.md), [Showcase](SHOWCASE.md) |
| Renderer i18n guardrails | Implemented | UI copy is centralized under `renderer/src/i18n/`; the renderer build checks Chinese/English key parity before bundling. | [Testing](TESTING.md) |
