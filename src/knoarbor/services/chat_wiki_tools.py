...
from knoarbor.services.chat_evidence import search_result_to_chat_payload
...
def query_wiki(context: ChatToolContext, arguments: dict[str, Any]) -> ChatToolTraceItem:
    query = _required_text(arguments, "query")
    max_results = _bounded_int(arguments.get("max_results"), default=6, minimum=1, maximum=12)
    requested_vault_id = _concrete_argument_vault_id(arguments, context.request.vault_id)
    request = WikiSearchRequest(
        config_path=context.request.config_path,
        vault_path=context.request.vault_path,
        vault_id=requested_vault_id,
        vault_ids=[str(item) for item in arguments.get("vault_ids", context.request.vault_ids) or []],
        all_vaults=bool(arguments.get("all_vaults", context.request.all_vaults or context.request.vault_id == VIRTUAL_ALL_VAULT_ID)),
        query=query,
    ...
    response = context.services.wiki_search.search(request)
    ...
    result["evidence_pack"] = context.evidence_planner.build_search_pack(
        query=response.query,
        result_count=len(response.results),
...
def list_wiki_pages(context: ChatToolContext, arguments: dict[str, Any]) -> ChatToolTraceItem:
    ...
    vaults = _resolve_tool_vaults(context, arguments)
    grouped_pages: list[dict[str, Any]] = []
    citations: list[ChatCitation] = []
    total_pages = 0
    for vault in vaults:
        response = context.services.wiki_pages.list_pages(vault.path, vault_id=vault.vault_id, vault_name=vault.vault_name)
...
def read_wiki_page(context: ChatToolContext, arguments: dict[str, Any]) -> ChatToolTraceItem:
    ...
    page_path = _answer_page_for_source_read(page_path, latest_user_text(context.request.messages), context.existing_session) or page_path
    requested_vault_id = _concrete_argument_vault_id(arguments, context.request.vault_id)
    if requested_vault_id is None:
        requested_vault_id = _vault_id_for_prior_page(page_path, context.existing_session)
    vault = resolve_single_vault(
        context.request.vault_path,
        requested_vault_id,
        context.request.config_path,
    )
    if _needs_page_reference_resolution(page_path):
        resolved_page_path = _resolve_page_reference(context.services, vault, page_path)
        if resolved_page_path:
            page_path = resolved_page_path
    page = context.services.wiki_pages.read_page(vault.path, page_path, vault_id=vault.vault_id, vault_name=vault.vault_name)
...
def inspect_wiki_relations(context: ChatToolContext, arguments: dict[str, Any]) -> ChatToolTraceItem:
    page_path = str(arguments.get("page_path") or arguments.get("path") or "").strip()
    if not page_path:
    ...
    if requested_vault_id is None:
        requested_vault_id = _vault_id_for_prior_page(page_path, context.existing_session)
    vault = resolve_single_vault(context.request.vault_path, requested_vault_id, context.request.config_path)
    relations = context.services.wiki_pages.page_relations(vault.path, page_path, vault_id=vault.vault_id, vault_name=vault.vault_name)
...
def with_default_query(arguments: dict[str, Any], query: str) -> dict[str, Any]:
...
def _resolve_tool_vaults(context: ChatToolContext, arguments: dict[str, Any]):
    vault_id = _concrete_argument_vault_id(arguments, context.request.vault_id)
    vault_ids = [str(item) for item in arguments.get("vault_ids", context.request.vault_ids) or [] if str(item).strip()]
    all_vaults = bool(arguments.get("all_vaults", context.request.all_vaults or context.request.vault_id == VIRTUAL_ALL_VAULT_ID))
