...
- Plan user-facing wiki page operations for one `source_digest.v1` plus optional `knowledge_atoms.v2`.
...
- Scope excludes page body writing and source digest audit page writing.
...
- `canonical_path` is the durable page path relative to the wiki content root. New user-facing knowledge pages use `pages/<Title>.md`.
- `page_dir` is the physical write-location field. Use `"pages"` for actionable knowledge pages, or null for skip.
- Source digest audit pages are committed by deterministic ingest postprocess from `source_digest.v1`; they are outside this agent's output operations.
    ...
    "operations": [
      {
        ...
        "page_dir": "pages",
        "canonical_path": "pages/Title.md or null",
...
- If the normalized source came from a segmented long source, keep page boundaries stable rather than segment-local. Keep page boundaries stable across the whole source.
- For create operations, set `canonical_path` to `pages/<Title>.md`.
- For update operations, set `canonical_path` to the target page's canonical path when known; otherwise use `target_page`.
- For all create and update operations, set `page_dir` to `"pages"`.
...
- If `knowledge_atoms` is empty, return `skip`; knowledge pages require selected claim atom ids.
