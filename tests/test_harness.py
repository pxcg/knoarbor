...
from tests.harness.semantic_cases import markdown_source_document, source_normalize_output
...
class HarnessTests(unittest.TestCase):
    def test_scripted_chat_client_records_requests_and_outputs_json(self) -> None:
        client = ScriptedChatClient.single(source_normalize_output(title="Harness Unit"))

        result = SemanticRunner(client).run("source_normalize", {"source_document": {"title": "Harness Unit"}})

        self.assertEqual(client.calls, 1)
        self.assertEqual(result.output.source.title, "Harness Unit")
