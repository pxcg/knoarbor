...
class CheckpointStoreTests(unittest.TestCase):
    ...
    def test_source_checkpoint_reprocesses_when_generated_wiki_page_is_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            vault = Path(tmp_dir)
            source_path = vault / "raw" / "notes" / "note.md"
            page_path = vault / "wiki" / "pages" / "Demo.md"
            source_path.parent.mkdir(parents=True)
            ...
            unchanged = store.prepare_source_file(vault, state, source_path, connector_version="markdown@1", parser_version="markdown-parser@1")
            page_path.unlink()
            missing = store.prepare_source_file(vault, state, source_path, connector_version="markdown@1", parser_version="markdown-parser@1")

        self.assertEqual(unchanged.mode, "unchanged")
        self.assertFalse(unchanged.should_process)
        self.assertEqual(missing.mode, "output_missing")
        self.assertTrue(missing.should_process)
        self.assertIn("Demo.md", missing.reason)

    def test_source_checkpoint_reprocesses_when_source_digest_is_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            vault = Path(tmp_dir)
            source_path = vault / "raw" / "notes" / "note.md"
            digest_path = vault / "wiki" / "sources" / "Note-Source.md"
            source_path.parent.mkdir(parents=True)
            digest_path.parent.mkdir(parents=True)
            source_path.write_text("# Note\n", encoding="utf-8")
            digest_path.write_text("# Note Source\n", encoding="utf-8")
            store = CheckpointStore()
            state = store.read_state(vault / ".knoarbor" / "checkpoints" / "source_ingest_checkpoints.json")
            ...
            store.commit_source(
                ...
                source_file=first.source_file,
                content_hash=first.content_hash or "",
                generated_pages=["sources/Note-Source.md"],
                connector_version="markdown@1",
                parser_version="markdown-parser@1",
            )
            digest_path.unlink()
            second = store.prepare_source_file(vault, state, source_path, connector_version="markdown@1", parser_version="markdown-parser@1")

        self.assertEqual(second.mode, "output_missing")
        self.assertTrue(second.should_process)
        self.assertIn("sources/Note-Source.md", second.reason)

    def test_session_checkpoint_reprocesses_full_session_when_output_is_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            vault = Path(tmp_dir)
            session_path = vault / "raw" / "chats" / "session_demo.json"
            page_path = vault / "wiki" / "pages" / "Demo.md"
            ...
            first = store.prepare_session_payload(vault, state, session_path, payload, connector_version="chat@1", parser_version="chat-parser@1")
            store.commit_session(
                vault,
                state,
                session_id=first.session_id,
            ...
            page_path.unlink()
            second = store.prepare_session_payload(vault, state, session_path, payload, connector_version="chat@1", parser_version="chat-parser@1")
