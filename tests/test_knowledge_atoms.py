...
class KnowledgeAtomSchemaTest(unittest.TestCase):
    ...
    def test_atom_batch_summary_counts_unique_evidence(self) -> None:
        ...
        batch = KnowledgeAtomBatch(
            source_digest_id="sd_ios_audio_001",
            entities=[
                KnowledgeAtomObject(object_type="knowledge_object", name="SoundAnalysis", atom_id="entity_soundanalysis"),
                KnowledgeAtomObject(object_type="knowledge_object", name="iOS Audio ML Workflow", atom_id="entity_ios_audio_ml_workflow"),
            ],
            claims=[claim],
            relations=[relation],
            evidence=[evidence],
