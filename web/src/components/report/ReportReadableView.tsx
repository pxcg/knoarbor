...
function buildPageArtifacts(
  pages: string[],
  changes: Array<{ page: string; action?: string; summary?: string; diff: string[] }>,
): Array<{ path: string; changes: Array<{ page: string; action?: string; summary?: string; diff: string[] }> }> {
  const byPath = new Map<string, Array<{ page: string; action?: string; summary?: string; diff: string[] }>>();
...
function getReportMetricNumber(content: string, key: string) {
...
function parseReport(content: string) {
  ...
  const sections: Array<{ title: string; items: Array<{ key?: string; value: string }> }> = [];
  let currentSection: { title: string; items: Array<{ key?: string; value: string }> } | null = null;
  let inFence = false;
...
function parseReportArtifacts(content: string) {
  const lines = content.split(/\r?\n/);
  const pages = new Set<string>();
  const changes: Array<{ page: string; action?: string; summary?: string; diff: string[] }> = [];
  let activeChange: { page: string; action?: string; summary?: string; diff: string[] } | null = null;
  let inDiff = false;
...
function extractWikiPagePaths(value: string): string[] {
...
function isStructuredReportLine(value: string) {
  return (
